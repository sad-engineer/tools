...
from service_for_my_projects import Cataloger, logged
...
from tools_old.obj.data_preparers import ToolDataPreparer
from tools_old.obj.entities import ErrorWithData
    ...
    def decorator(func):
        def wrapper(self, *args, **kwargs):
            ...
            if isinstance(result.err, ValueError):
                ...
                self.error(
                    f"Ошибка создания экземпляра класса {result.name} с параметрами {result.params}."
                    f"Данные, полученные из БД: {result.raw_data}."
                )
...
class ToolCreator:
    ...
    @debug_for_init()
    def __init__(
        self, catalog: Cataloger, preparer_factory: Callable[..., ToolDataPreparer], finder: Callable[..., ToolFinder]
    ) -> None:
