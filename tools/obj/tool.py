...
from tools.obj.fields_types import InGroupsTool, StringValue, MarkingForSpecialTool


class Tool(BaseModel, Dictionarer):
    ...
    group: Optional[InGroupsTool] = None
    marking: Optional[StringValue] = None
    ...
    @property
    def name(self) -> str:
        return " ".join([self.group, self.marking.lower(), self.standard]) if isinstance(self._name, type(None)) else self._name
