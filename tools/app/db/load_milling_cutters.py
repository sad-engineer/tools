...
# ---------------------------------------------------------------------------------------------------------------------
"""
Скрипт для загрузки данных фрез из CSV файла в таблицу milling_cutters.

Этот скрипт:
1. Читает CSV файл с данными фрез
2. Фильтрует только строки с типом "Фреза"
3. Загружает данные в таблицу milling_cutters
4. Связывает фрезы с соответствующими инструментами из таблицы tools
    через общий id (MillingCutter.id == Tool.id)
"""
...
# Маппинг колонок CSV на поля модели MillingCutter
...
def load_milling_cutters_from_csv(csv_file_path: str, session: Session) -> int:
    """
    Загружает данные фрез из CSV файла в таблицу milling_cutters.
    ...
    with open(csv_path, 'r', encoding='utf-8') as file:
        ...
        for row_num, row in enumerate(reader, start=2):  # Начинаем с 2, так как 1 - заголовок
            try:
                ...
                # Теперь проверяем по id, так как MillingCutter.id == Tool.id
                existing_cutter = session.query(MillingCutter).filter(MillingCutter.id == tool.id).first()
                if existing_cutter:
                ...
                for csv_col, model_field in MILLING_CUTTER_COLUMN_MAPPING.items():
                    if csv_col in row:
                        ...
                        field_type = getattr(MillingCutter, model_field).type
                ...
                # Устанавливаем id фрезы равным id инструмента
                cutter_data['id'] = tool.id
                
                # Создаем и сохраняем фрезу
                cutter = MillingCutter(**cutter_data)
                session.add(cutter)
...
def main():
    ...
    try:
        ...
        with get_session() as session:
            loaded_count = load_milling_cutters_from_csv(csv_file_path, session)
