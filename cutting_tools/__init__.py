...
class RawDataChecker:
    DEFAULT_SETTINGS_FOR_CUTTING_TOOL = DEFAULT_SETTINGS_FOR_CUTTING_TOOL
    def __init__(self, raw_data: dict):
        self.raw_data = raw_data

    def check_and_create_params(self, cutter_tool_cls):
        param=dict()
        param["group"] = self.raw_data.get('group')
        param["marking"] = self.raw_data.get('marking')
        ...
        param["dia_mm"] = self.raw_data.get('D', 25)
        param["length_mm"] = self.raw_data.get('L', 25)
        param["main_angle_grad"] = self.raw_data.get('fi_', 45)
        param["front_angle_grad"] = self.raw_data.get('gamma_', 0)
        param["inclination_of_main_blade_grad"] = self.raw_data.get('lambda_', 0)
        param["type_cutter"] = self.raw_data.get('type_cutter')
        param["type_of_cutting_part"] = self.raw_data.get('type_of_cutting_part')
        param["num_of_cutting_blades"] = self.raw_data.get('z', 2)
        param["radius_of_cutting_vertex"] = self.raw_data.get('r_', 1.0)
        param["large_tooth"] = self.raw_data.get('large_tooth', 0)
        param["quantity"] = self.raw_data.get('quantity', 1)
