...
# -------------------------------------------------------------------------------
# Name:        cutting_tool
# Purpose:     Parameters of the cutting tool used
#
# Author:      ANKorenuk
#
# Created:     09.04.2022
...
class CuttingTool():
    ...
    Parameters (используемые в рассчетах)
    ----------
    ...
    def __init__(self,
        ...
        self.standart = standart                                # Стандарт инструмента
    ...
    def get_settings(self, marking, standart, mat_of_cutting_part=None, accuracy=None) -> None:
        ...
        params = by_marking_and_stand(marking, standart)
        if not isinstance(accuracy, type(None)):
            params["Точность"] = accuracy
        self.name = get_name(params, mat_of_cutting_part)
        self.type_cutter = index_tc(params['type_cutter_']) if not isinstance(params['type_cutter_'], type(None)) else None
        ...
        self.type_of_cutting_part = int(params['type_of_cutting_part_']) if not isinstance(params['type_of_cutting_part_'], type(None)) else None
        self.main_angle_grad = float(params['fi_']) if not isinstance(params['fi_'], type(None)) else None
        self.front_angle_grad = float(params['gamma_']) if not isinstance(params['gamma_'], type(None)) else None               # gamma передний угол в плане
        self.inclination_of_main_blade = float(params['lambda_']) if not isinstance(params['lambda_'], type(None)) else None    # lambda угол наклона переднего лезвия
        self.radius_of_cutting_vertex = float(params['r_']) if not isinstance(params['r_'], type(None)) else None               # r радиус режущей вершины
