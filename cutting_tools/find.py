...
def by_dia_and_type(dia: float = None, dia_out: float = None, type_tool: str = None,
    ...
    Parameters:
        dia : Диаметр инструмента
        dia_out : Диаметр инструмента. (Для насадных инструментов)
        type_tool : Тип инструмента (Сверло, резец, и т.д.)
        path_bd : Путь к базе данных по материалам

    Returns:
        Возвращает коэффициент и показатели степени для расчета крутящего момента.
    """
...
def by_marking(marking: str = "2300-0041", path_bd: str = PATH_DB) -> pd.DataFrame:
    ...
    Parameters:
        marking : Наименование инструмента
        path_bd : Путь к базе данных по материалам

    Returns:
        Таблица найденного инструмента по обозначению. Внимание! Таблица может содержать более одной строки.
    """
    db, cursor = connect(path_bd)
...
def full_table(table_name: str = "cutting_tools", path_bd: str = PATH_DB) -> pd.DataFrame:
