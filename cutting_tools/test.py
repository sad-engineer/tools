...
class ToolData:
    ...
    def group(self, group):
        if isinstance(group, (int, float)) and group not in self.GROUPS_TOOL.values():
            raise ValueError(f'Неверное значение индекса группы инструмента. Значение должно быть из '
                             f'{self.GROUPS_TOOL}.')
        if isinstance(group, str) and group not in self.GROUPS_TOOL:
            raise ValueError(f'Неверное значение группы инструмента. Значение должно быть из {self.GROUPS_TOOL}.')
    ...
    def standard(self, any_standard):
        if any_standard.split(" ")[0] not in self.TYPES_STANDARD:
            raise ValueError(f'Неверное значение стандарта инструмента.')
        if not any_standard.find("-") != -1:
            raise ValueError(f'Неверное значение стандарта инструмента (возможно не указан год).')
...
obj = ToolData(group=0, )
print(obj.group)
print(obj.name)

obj = ToolData(group="Фреза", )
print(obj.group)
print(obj.name)
